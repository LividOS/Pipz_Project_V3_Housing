...
; Content-Fingerprint: 2026-01-29T21-41-01Z-OM5HO76J
; ------------------------------------------------------------------
...
;       * g_AntiBanEnabled (mandatory gate for AntiBanCheckpoint)
;
...
AntiBanCheckpoint() {
    global g_AntiBanEnabled, g_WorkerState

    if (!IsSet(g_AntiBanEnabled) || !g_AntiBanEnabled)
        return

    if (IsSet(g_WorkerState) && (g_WorkerState != "active"))
        return
...
DoBreak(minMs, maxMs) {
    ...
    if (duration > 5000) {
        try {
            if IsSet(UpdateWorkerOverlay) && (UpdateWorkerOverlay is Func)
                UpdateWorkerOverlay("Status - Break: " . Round(duration/1000) . "s", "Orange")
        }
